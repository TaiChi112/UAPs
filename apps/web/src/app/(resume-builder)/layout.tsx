      ...
      <div className="flex h-16 items-center justify-between px-6 max-w-[1400px] mx-auto">
        <div className="flex items-center gap-10">
          ...
          <nav className="hidden md:flex items-center gap-1">
            <Link href="/" className="px-4 py-2 text-sm font-medium text-slate-600 hover:text-blue-600 hover:bg-blue-50/80 rounded-lg transition-colors">
              Public Feed
            </Link>
            {user && (
              <Link href="/vault" className="px-4 py-2 text-sm font-medium text-slate-600 hover:text-blue-600 hover:bg-blue-50/80 rounded-lg transition-colors">
                My Vault
              </Link>
            )}
          </nav>
          ...
          {user && (
            <div className="flex items-center gap-4 pl-4 border-l border-slate-200">
              ...
              <div className="flex items-center gap-2">
                <div className="flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br from-blue-100 to-indigo-100 border border-blue-200/60 text-sm font-bold text-blue-700 shadow-sm ring-2 ring-white">
                  {user.name.split(" ").map((n) => n[0]).join("").slice(0, 2).toUpperCase() || <UserIcon className="h-4 w-4" />}
                </div>
                <a
                  href="/auth/logout"
                  className="flex h-10 w-10 items-center justify-center rounded-full text-slate-400 hover:bg-red-50 hover:text-red-600 transition-colors ml-1"
                  title="Sign Out"
                >
