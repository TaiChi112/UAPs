...
import { 
  ...
  Eye, Edit2, Copy, Trash2, FileDown, Tag 
} from 'lucide-react';
...
const INITIAL_RESUMES = [
  {
    ...
    config: {
      ...
      summary: "Passionate AI Engineer aiming to leverage machine learning skills.",
  ...
  },
  {
    ...
    config: {
      ...
      summary: "Detail-oriented analyst with a strong background in data cleaning.",
      ...
      <div className="space-y-6 flex-1">
        ...
        {previewData.skills.length === 0 && previewData.projects.length === 0 && previewData.experience.length === 0 && previewData.certificates.length === 0 && (
          <div className="text-center py-12 text-slate-400">
            ...
            <p className="text-sm">Start selecting or adding information to build it.</p>
          </div>
        )}
      </div>
    </div>
        ...
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
            <div>
              ...
              <p className="text-slate-500 mt-2">Manage, duplicate, and track your tailored resumes.</p>
            </div>
            <button onClick={handleStartNew} className="bg-blue-600 text-white px-5 py-3 rounded-xl font-medium flex items-center gap-2 hover:bg-blue-700 transition-colors shadow-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {savedResumes.map(res => {
              ...
              return (
                <div key={res.id} className="bg-white rounded-2xl shadow-sm border border-slate-200 hover:shadow-md transition-all group flex flex-col justify-between overflow-hidden">
                  
                  {/* Card Content (Click to Preview) */}
                  <div className="p-6 pb-4 cursor-pointer" onClick={() => setPreviewResume(res)}>
                  ...
                  <div className="border-t border-slate-100 bg-slate-50/50 p-3 px-4 flex justify-between items-center">
                    ...
                    <button onClick={() => setPreviewResume(res)} className="text-sm font-medium text-slate-600 flex items-center gap-1 hover:text-slate-900 bg-white px-3 py-1.5 rounded-lg border border-slate-200 shadow-sm">
            ...
            {savedResumes.length === 0 && (
              <div className="col-span-full py-12 text-center text-slate-400 bg-white border border-dashed border-slate-300 rounded-2xl">
                <FileText className="w-12 h-12 mx-auto mb-3 opacity-20" />
                <p>No resumes yet.</p>
                <button onClick={handleStartNew} className="text-blue-600 mt-2 font-medium hover:underline">Create your first one</button>
              </div>
            )}
          </div>
        </div>
          ...
          <div className="fixed inset-0 z-50 flex justify-center bg-slate-900/60 backdrop-blur-sm p-4 md:p-8">
            ...
            <div className="relative w-full max-w-5xl h-full flex flex-col bg-slate-100 rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
              ...
              <div className="bg-white px-6 py-4 border-b flex justify-between items-center sticky top-0 z-10 shadow-sm">
                <div>
                  <h2 className="text-xl font-bold text-slate-800">{previewResume.title}</h2>
                  <p className="text-sm text-slate-500 flex items-center gap-2">
                    <Calendar className="w-3 h-3"/> Last edited: {previewResume.date}
                  </p>
                </div>
                <div className="flex gap-3">
                  <button onClick={() => { setPreviewResume(null); handleEdit(previewResume); }} className="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 font-medium rounded-lg text-sm flex items-center gap-2 transition-colors">
                    <Edit2 className="w-4 h-4"/> Edit Document
                  </button>
                  <button onClick={handleDownloadMock} className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg text-sm flex items-center gap-2 transition-colors shadow-sm">
                    <FileDown className="w-4 h-4"/> Download PDF
                  </button>
                  <button onClick={() => setPreviewResume(null)} className="p-2 hover:bg-slate-100 text-slate-500 rounded-lg transition-colors ml-2">
                    <X className="w-6 h-6"/>
                  </button>
                </div>
              </div>
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden flex flex-col h-[90vh]">
          <div className="bg-slate-50 border-b border-slate-200 p-4 flex justify-between items-center">
            ...
            <button onClick={handleSaveResume} className="bg-slate-900 text-white px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2 hover:bg-slate-800 transition-colors shadow-sm">
          ...
          <div className="p-6 overflow-y-auto flex-1 space-y-8">
            ...
            <section className="space-y-3">
              ...
              <form onSubmit={handleAddNewSkill} className="flex gap-2 mt-2">
                <input type="text" placeholder="Type new skill & press Enter..." className="flex-1 text-sm p-2 border rounded-md focus:ring-2 focus:ring-blue-500 outline-none" value={newSkill} onChange={e => setNewSkill(e.target.value)} />
