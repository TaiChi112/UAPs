...
// 2. Mock Data สำหรับ Resume ที่เคยสร้างเก็บไว้ (Dashboard)
...
export default function ResumeBuilder() {
  // State 0: การจัดการหน้า (Dashboard vs Builder)
  ...
  // --- Functions: การจัดการหน้าต่างและ Save (Create & Read) ---
  const handleStartNew = () => {
    // เคลียร์ข้อมูลหน้ากระดาษก่อนเริ่มสร้างฉบับใหม่
    setResumeConfig({
  ...
  const handleSaveResume = () => {
    // สร้างชื่อ Resume แบบง่ายๆ จาก Role และ Company (ถ้าไม่ได้กรอกให้ใช้ชื่อ Default)
    ...
    const newResume = {
      ...
      role: resumeConfig.targetRole,
      company: resumeConfig.targetCompany
    };

    // 1. เพิ่มเข้าคลัง Resumes (เซฟ)
    setSavedResumes([newResume, ...savedResumes]); 
    // 2. กลับไปหน้า Dashboard
  ...
  // =========================================================
  // VIEW 1: DASHBOARD (หน้ารวม Resume ที่สร้างแล้ว)
  // =========================================================
        ...
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
            <div>
              ...
              <p className="text-slate-500 mt-2 text-sm md:text-base">Manage your tailored resumes for different roles and companies.</p>
            </div>
            <button 
              onClick={handleStartNew}
              className="bg-blue-600 text-white px-5 py-3 rounded-xl font-medium flex items-center gap-2 hover:bg-blue-700 transition-colors shadow-sm hover:shadow-md w-full md:w-auto justify-center"
            >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {savedResumes.map(res => (
              <div key={res.id} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 hover:shadow-md transition-shadow group flex flex-col justify-between h-48 cursor-default">
                <div>
                  <div className="flex justify-between items-start mb-2">
                    <div className="w-10 h-10 rounded-lg bg-blue-50 flex items-center justify-center text-blue-600 transition-colors">
                  ...
                  <h3 className="font-bold text-lg text-slate-900 mt-4 line-clamp-1">{res.title}</h3>
                  <p className="text-sm text-slate-500 mt-1 line-clamp-1">
                    {res.role || "General Role"} • {res.company || "Any Company"}
                  </p>
                </div>
                <div className="text-slate-400 text-sm font-medium flex items-center gap-1 opacity-70">
                  <CheckCircle className="w-4 h-4 text-green-500" /> Saved Successfully
                </div>
              </div>
            ))}
          </div>
  ...
  // =========================================================
  // VIEW 2: BUILDER (หน้าสร้าง Resume)
  // =========================================================
  return (
